...
## Approach 1 (Active): Inline Single-Pass

> **Key Insight:** Extract each digit using `n % 10` and accumulate the sum and product simultaneously in a single loop. After the loop, compute `div = sum + prod` and check `n % div == 0`. A guard `div != 0` handles the edge case where a digit is `0` making the product (and thus `div`) zero.
...
> **Key Insight:** Same logic as Approach 1, but delegates digit extraction to a private helper method `getSumAndProd(n)` that returns the combined divisor. This improves readability and separates concerns.
